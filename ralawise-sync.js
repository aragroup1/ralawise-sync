...
const config = {
  ...
  review: {
    // Thresholds that trigger review (not failsafe)
    inventoryChangeReview: parseInt(process.env.INVENTORY_CHANGE_REVIEW || '5'), // % change that needs review
    newProductsReview: parseInt(process.env.NEW_PRODUCTS_REVIEW || '10'), // Number of new products that need review
    discontinueReview: parseInt(process.env.DISCONTINUE_REVIEW || '10'), // Number to discontinue that needs review
    priceChangeReview: parseInt(process.env.PRICE_CHANGE_REVIEW || '20'), // % price change that needs review
  }
};
...
async function notifyTelegram(message, isUrgent = false, options = {}) {
...
async function requestReview(operation, details, callback) {
  ...
  pendingReviews.set(requestId, review);
  ...
  addLog(`📋 REVIEW REQUIRED: ${operation} (ID: ${requestId})`, 'warning');
  
  // Build detailed message
  let message = `<b>📋 Review Required</b>\n\n`;
  message += `<b>Operation:</b> ${operation}\n`;
  ...
  if (details.samples && details.samples.length > 0) {
    message += `<b>Sample Items:</b>\n`;
    details.samples.slice(0, 5).forEach(sample => {
      message += `• ${sample}\n`;
    });
    if (details.samples.length > 5) {
      message += `• ... and ${details.samples.length - 5} more\n`;
    }
    message += '\n';
  }
  
  if (details.risks) {
    message += `<b>⚠️ Potential Risks:</b>\n`;
  ...
  message += `<b>Please review and respond:</b>\n`;
  message += `• Click "✅ Approve" to proceed\n`;
  message += `• Click "❌ Reject" to cancel\n`;
  message += `• Or use dashboard for more details\n\n`;
  message += `⏱️ This request will timeout in 10 minutes`;
  
  // Send Telegram notification with buttons
  const telegramMessage = await notifyTelegram(message, true, {
    buttons: [
      [
      ...
      [
        { text: '📊 View Dashboard', url: process.env.RAILWAY_STATIC_URL || `http://localhost:${config.port || 3000}` }
      ]
    ]
  });
  
  // Set timeout (10 minutes)
  review.timeout = setTimeout(() => {
    if (pendingReviews.has(requestId)) {
      handleReviewResponse(requestId, false, 'Timeout - no response received');
    }
  }, 10 * 60 * 1000);
...
function handleReviewResponse(requestId, approved, reason = '') {
  const review = pendingReviews.get(requestId);
  if (!review || review.status !== 'pending') {
    addLog(`Review ${requestId} not found or already processed`, 'warning');
    return false;
  }
  ...
  review.responseTime = new Date().toISOString();
  review.responseReason = reason;
  
  // Clear review state if this is the current review
  if (reviewState.requestId === requestId) {
    reviewState = {
      pending: false,
      operation: null,
      requestId: null,
  ...
  // Log the decision
  addLog(`Review ${requestId} ${approved ? 'APPROVED' : 'REJECTED'}: ${reason || 'No reason provided'}`, approved ? 'success' : 'warning');
  
  // Send notification
  notifyTelegram(`${approved ? '✅' : '❌'} Review ${approved ? 'Approved' : 'Rejected'}\n\nOperation: ${review.operation}\nID: ${requestId}\n${reason ? `Reason: ${reason}` : ''}`);
  
  // Execute callback if exists
  if (review.callback) {
    review.callback(approved, reason);
  }
  
  // Resolve the promise
  if (review.resolver) {
    review.resolver(approved);
  }
  
  return true;
}

function checkReviewNeeded(context, data) {
  const reviews = [];
  
  switch (context) {
    case 'inventory': {
      const changePercentage = (data.updateCount / Math.max(data.totalProducts, 1)) * 100;
      if (changePercentage > config.review.inventoryChangeReview && changePercentage <= config.failsafe.maxChangePercentage) {
        reviews.push({
          reason: `Inventory changes affect ${changePercentage.toFixed(1)}% of products`,
          details: data
        });
      }
      break;
    ...
    case 'products': {
      if (data.toCreate >= config.review.newProductsReview && data.toCreate < config.failsafe.maxProductsToCreate) {
        reviews.push({
          reason: `Creating ${data.toCreate} new products`,
          details: data
        });
      }
      if (data.toDiscontinue >= config.review.discontinueReview && data.toDiscontinue < config.failsafe.maxProductsToDiscontinue) {
        reviews.push({
          reason: `Discontinuing ${data.toDiscontinue} products`,
          details: data
        });
      }
      break;
    ...
    case 'price': {
      if (data.priceChanges && data.priceChanges.length > 0) {
        const significantChanges = data.priceChanges.filter(change => 
          Math.abs(change.percentChange) > config.review.priceChangeReview
        );
        if (significantChanges.length > 0) {
          reviews.push({
            reason: `${significantChanges.length} products have significant price changes`,
            details: { ...data, significantChanges }
          });
        }
      }
      break;
...
function pauseSystem(reason = 'Manual pause', pausedBy = 'Dashboard') {
  if (pauseState.paused) {
    return { success: false, message: 'System is already paused' };
  }
  ...
  addLog(`⏸️ SYSTEM PAUSED: ${reason} (by ${pausedBy})`, 'warning');
  
  // Send Telegram notification
  notifyTelegram(`⏸️ <b>System Paused</b>\n\nReason: ${reason}\nPaused by: ${pausedBy}\nTime: ${new Date().toLocaleString()}\n\nAll scheduled and manual operations are now paused.`);
  
  return { success: true, message: 'System paused successfully' };
}

function resumeSystem(resumedBy = 'Dashboard') {
  if (!pauseState.paused) {
    return { success: false, message: 'System is not paused' };
  }
  
  const previousReason = pauseState.reason;
  const pauseDuration = pauseState.timestamp ? 
    Math.round((Date.now() - new Date(pauseState.timestamp).getTime()) / 1000 / 60) : 0;
  ...
  addLog(`▶️ SYSTEM RESUMED (by ${resumedBy}) - was paused for ${pauseDuration} minutes`, 'success');
  
  // Send Telegram notification
  notifyTelegram(`▶️ <b>System Resumed</b>\n\nResumed by: ${resumedBy}\nPrevious pause reason: ${previousReason}\nPause duration: ${pauseDuration} minutes\n\nAll operations can now proceed.`);
...
function getBlockReason() {
  if (failsafeState.triggered) {
    return `Failsafe: ${failsafeState.reason}`;
  }
  if (pauseState.paused) {
    return `Paused: ${pauseState.reason}`;
  }
  if (reviewState.pending) {
    return `Review Pending: ${reviewState.operation}`;
  }
  return null;
}
...
async function updateInventoryBySKU(inventoryMap) {
  ...
  if (isSystemBlocked()) {
    ...
    addLog(`Inventory update blocked: ${reason}`, 'warning');
  ...
  try {
    ...
    if (!isSystemBlocked() && shopifyProducts.length > config.failsafe.minExpectedProducts) {
    ...
    const changePercentage = (inventoryUpdates.length / Math.max(totalRalawiseProducts, 1)) * 100;
    
    const reviewNeeded = checkReviewNeeded('inventory', {
      updateCount: inventoryUpdates.length,
      totalProducts: totalRalawiseProducts,
      changePercentage
    });
    
    if (reviewNeeded) {
      const approved = await requestReview('Inventory Update', {
        summary: `${inventoryUpdates.length} products (${changePercentage.toFixed(1)}%) need inventory updates`,
        changes: {
          'Products to update': inventoryUpdates.length,
          'Total products': totalRalawiseProducts,
          'Change percentage': `${changePercentage.toFixed(1)}%`,
          'SKUs not found': notFound
        },
        samples: inventoryUpdates.slice(0, 10).map(u => 
          `${u.title}: ${u.currentInventory} → ${u.newInventory} (${u.difference > 0 ? '+' : ''}${u.difference})`
        ),
        risks: changePercentage > 8 ? 
          [`Large inventory change affecting ${changePercentage.toFixed(1)}% of products`] : 
          []
      });
      
      if (!approved) {
        addLog('Inventory update rejected by user', 'warning');
        isRunning.inventory = false;
        return { blocked: true, reason: 'User rejected update' };
      }
    }
    ...
    for (const update of inventoryUpdates) {
      ...
      if (isSystemBlocked()) {
        addLog('System blocked during inventory update, stopping...', 'warning');
        break;
      }
...
async function processFullImport(csvFiles) {
  ...
  if (isSystemBlocked()) {
    const reason = getBlockReason();
    addLog(`Full import blocked: ${reason}`, 'warning');
    return { blocked: true, reason };
  }
  ...
  try {
    ...
    // Check for review requirement
    const reviewNeeded = checkReviewNeeded('products', {
      toCreate: toCreate.length,
      toDiscontinue: toDiscontinue.length,
      existingCount: ralawiseProducts.length
    });
    
    if (reviewNeeded) {
      const approved = await requestReview('Full Product Import', {
        summary: `Create ${toCreate.length} products, discontinue ${toDiscontinue.length} products`,
        changes: {
          'New products to create': toCreate.length,
          'Products to discontinue': toDiscontinue.length,
          'Existing Ralawise products': ralawiseProducts.length,
          'Total products in import': productsByHandle.size
        },
        samples: [
          ...toCreate.slice(0, 3).map(p => `Create: ${p.title} (${p.variants.length} variants)`),
          ...toDiscontinue.slice(0, 3).map(p => `Discontinue: ${p.title}`)
        ],
        risks: [
          toCreate.length > 20 ? `Creating ${toCreate.length} products may take several minutes` : null,
          toDiscontinue.length > 20 ? `Discontinuing ${toDiscontinue.length} products will set them to draft` : null
        ].filter(Boolean)
      });
      
      if (!approved) {
        addLog('Full import rejected by user', 'warning');
        isRunning.fullImport = false;
        return { blocked: true, reason: 'User rejected import' };
      }
    }
    ...
    for (const product of productsToCreate) {
      // Check if paused during execution
      if (isSystemBlocked()) {
        addLog('System blocked during product creation, stopping...', 'warning');
        break;
      }
    ...
    for (const product of productsToDiscontinue) {
      // Check if paused during execution
      if (isSystemBlocked()) {
        addLog('System blocked during product discontinuation, stopping...', 'warning');
        break;
      }
...
async function syncInventory() {
  try {
    if (isSystemBlocked()) {
      const reason = getBlockReason();
      addLog(`Sync blocked: ${reason}`, 'warning');
      return { blocked: true, reason };
    }
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (action === 'approve' || action === 'reject') {
        ...
        const success = handleReviewResponse(requestId, approved, `Telegram: ${update.callback_query.from.username || 'User'}`);
    ...
    addLog(`Telegram webhook error: ${error.message}`, 'error');
    res.json({ ok: true });
  }
});
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .failsafe-details, .pause-details {
          background: #fff3cd;
        ...
        .failsafe-details h4, .pause-details h4 {
          margin: 0 0 10px 0;
          color: #856404;
        }
        .failsafe-details pre, .pause-details pre {
          background: #f8f9fa;
          padding: 10px;
          border-radius: 4px;
          overflow-x: auto;
    ...
    <body>
      <div class="container">
        ...
        <div class="alert review">
          📋 REVIEW PENDING - Operation waiting for approval
        ...
        <div class="alert">
          ⚠️ FAILSAFE ACTIVE - All operations are blocked
        ...
        <div class="alert warning">
          ⏸️ SYSTEM PAUSED - All operations are on hold
          <button onclick="resumeSystem()">Resume System</button>
        </div>
        <div class="card pause-details">
          <h4>Pause Details</h4>
          <p><strong>Reason:</strong> ${pauseState.reason}</p>
          <p><strong>Paused by:</strong> ${pauseState.pausedBy}</p>
          <p><strong>Paused at:</strong> ${pauseState.timestamp ? new Date(pauseState.timestamp).toLocaleString() : 'Unknown'}</p>
        </div>
        ` : ''}
        
        ${pendingReviewsList.length > 0 ? `
        <div class="card">
          <h2>📋 Pending Reviews (${pendingReviewsList.length})</h2>
        ...
        <div class="card">
          ...
          <div class="stats">
            <div class="stat ${isRunning.inventory ? 'running' : ''} ${failsafeState.triggered ? 'error' : ''} ${pauseState.paused && !failsafeState.triggered ? 'warning' : ''} ${reviewState.pending ? 'review' : ''}">
              ...
              <small>${failsafeState.triggered ? 'Blocked by Failsafe' : (pauseState.paused ? 'Paused' : (reviewState.pending ? 'Awaiting Review' : (isRunning.inventory ? 'Running...' : 'Idle')))}</small>
            </div>
            <div class="stat ${isRunning.fullImport ? 'running' : ''} ${failsafeState.triggered ? 'error' : ''} ${pauseState.paused && !failsafeState.triggered ? 'warning' : ''} ${reviewState.pending ? 'review' : ''}">
              ...
              <small>${failsafeState.triggered ? 'Blocked by Failsafe' : (pauseState.paused ? 'Paused' : (reviewState.pending ? 'Awaiting Review' : (isRunning.fullImport ? 'Running...' : 'Idle')))}</small>
            </div>
            <div class="stat ${failsafeState.triggered ? 'error' : (pauseState.paused ? 'warning' : (reviewState.pending ? 'review' : ''))}">
              <h3>System State</h3>
              <p>${failsafeState.triggered ? '🚨' : (pauseState.paused ? '⏸️' : (reviewState.pending ? '📋' : '✅'))}</p>
              <small>${failsafeState.triggered ? 'Failsafe Active' : (pauseState.paused ? 'Manually Paused' : (reviewState.pending ? 'Review Pending' : 'Normal Operation'))}</small>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div style="margin-bottom: 20px;">
            ${!pauseState.paused && !failsafeState.triggered ? `
              <button onclick="pauseSystem()" class="pause">
                ⏸️ Pause System
              </button>
            ` : ''}
            ${pauseState.paused && !failsafeState.triggered ? `
              <button onclick="resumeSystem()" class="resume">
                ▶️ Resume System
              </button>
            ` : ''}
            ${failsafeState.triggered ? `
              <button onclick="clearFailsafe()" class="danger">
                ⚠️ Clear Failsafe
              </button>
            ` : ''}
          </div>
          
          <h3 style="margin: 20px 0 10px 0;">Manual Sync Controls</h3>
          <button onclick="runInventorySync()" id="invBtn" ${isSystemBlocked() ? 'disabled' : ''}>
            🔄 Run Inventory Sync Now
          </button>
          <button onclick="runFullImport()" id="fullBtn" ${isSystemBlocked() ? 'disabled' : ''}>
            📦 Run Full Import Now
          </button>
          ...
          <p style="margin-top: 20px; color: #666; font-size: 14px;">
            ...
            ${pauseState.paused || failsafeState.triggered || reviewState.pending ? '<span style="color: red;">• ⚠️ Scheduled tasks are currently suspended</span><br>' : ''}
            <br>
            <strong>Review Thresholds:</strong><br>
            • Inventory changes > ${config.review.inventoryChangeReview}%<br>
            • New products > ${config.review.newProductsReview}<br>
            • Discontinued products > ${config.review.discontinueReview}<br>
            <br>
      ...
      <script>
        ...
        // Auto-refresh every 30 seconds if a job is running or review pending
        const checkRefresh = () => {
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    pauseState,
    reviewState,
    pendingReviews: Array.from(pendingReviews.values()),
    systemBlocked: isSystemBlocked(),
...
app.post('/api/review/:id/approve', (req, res) => {
  const success = handleReviewResponse(req.params.id, true, 'Dashboard approval');
  res.json({ success, message: success ? 'Review approved' : 'Review not found' });
});

app.post('/api/review/:id/reject', (req, res) => {
  const { reason = 'No reason provided' } = req.body;
  const success = handleReviewResponse(req.params.id, false, `Dashboard: ${reason}`);
  res.json({ success, message: success ? 'Review rejected' : 'Review not found' });
});

app.post('/api/pause', (req, res) => {
  const { reason = 'Manual pause' } = req.body;
  const result = pauseSystem(reason, 'API');
  res.json(result);
});

app.post('/api/resume', (req, res) => {
  const result = resumeSystem('API');
  res.json(result);
});
...
app.post('/api/sync/inventory', async (req, res) => {
  if (isSystemBlocked()) {
    const reason = getBlockReason();
    return res.json({ error: `System is blocked: ${reason}` });
  }
...
cron.schedule('0 * * * *', () => {
  if (!isRunning.inventory && !isSystemBlocked()) {
    ...
    const reason = getBlockReason();
    addLog(`Scheduled inventory sync skipped - ${reason}`, 'warning');
  }
});

// Full import every 2 days at 13:00 UK time
cron.schedule('0 13 */2 * *', () => {
  if (!isRunning.fullImport && !isSystemBlocked()) {
    ...
    const reason = getBlockReason();
    addLog(`Scheduled full import skipped - ${reason}`, 'warning');
  }
}, {
...
app.listen(PORT, () => {
  ...
  addLog(`📋 Review: Inventory >${config.review.inventoryChangeReview}%, Products >${config.review.newProductsReview}`, 'info');
  addLog(`💰 Pricing: 0-6=×2.1, 7-11=×1.9, 11+=×1.75`, 'info');
  
  // Send startup notification
  notifyTelegram(`✅ System Started\n\nShopify: ${config.shopify.domain}\nFTP: ${config.ftp.host}\n\nReview thresholds:\n• Inventory: >${config.review.inventoryChangeReview}%\n• New products: >${config.review.newProductsReview}\n• Discontinue: >${config.review.discontinueReview}\n\nFailsafe limits:\n• Max change: ${config.failsafe.maxChangePercentage}%\n• Max create: ${config.failsafe.maxProductsToCreate}\n• Max discontinue: ${config.failsafe.maxProductsToDiscontinue}`);
  
  // Set up Telegram webhook if configured
  if (config.telegram.botToken && process.env.RAILWAY_STATIC_URL) {
    const webhookUrl = `${process.env.RAILWAY_STATIC_URL}/telegram/webhook`;
    axios.post(
      `https://api.telegram.org/bot${config.telegram.botToken}/setWebhook`,
      { url: webhookUrl }
    ).then(() => {
      addLog('Telegram webhook configured', 'success');
    }).catch(error => {
      addLog(`Failed to set Telegram webhook: ${error.message}`, 'warning');
    });
  }
