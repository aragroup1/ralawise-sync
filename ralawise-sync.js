...
const config = {
    ...
    ralawise: { 
        ...
        maxProductsPerRun: parseInt(process.env.MAX_PRODUCTS_PER_RUN || '0') // 0 = no limit
    },
    telegram: { 
...
let syncProgress = { 
    ...
    fullImport: { isActive: false, created: 0, discontinued: 0, updated: 0, current: 0, total: 0 },
...
// Process new products/updates ONLY (no discontinuation) - NO LIMIT
async function processProductImport() {
    ...
    syncProgress.fullImport = { isActive: true, created: 0, updated: 0, current: 0, total: 0, startTime: Date.now() };
    
    const finalizeRun = (status) => {
        runResult.status = status;
        const runtime = ((Date.now() - syncProgress.fullImport.startTime) / 1000 / 60).toFixed(1);
    ...
    try {
        ...
        syncProgress.fullImport.total = productsByBaseSKU.size;
        
        // Process each grouped product - NO LIMIT
        ...
        for (const [baseSKU, productData] of productsByBaseSKU) {
            try {
                ...
                processedCount++;
                updateProgress('fullImport', processedCount, productsByBaseSKU.size);
                
                // Log progress every 50 products
                if (processedCount % 50 === 0) {
                    addLog(`Progress: Processed ${processedCount}/${productsByBaseSKU.size} products...`, 'info');
                }
                
                // Optional: Apply limit only if configured
                if (config.ralawise.maxProductsPerRun > 0 && processedCount >= config.ralawise.maxProductsPerRun) {
                    addLog(`Reached configured limit of ${config.ralawise.maxProductsPerRun} products`, 'warning');
                    break;
                }
                ...
                weeklyReport.errors.push({ product: productData.title, error: e.message, timestamp: new Date().toISOString() });
            }
        }
        
        addLog(`Completed processing ${processedCount} products`, 'success');
        finalizeRun('completed');
    } catch (e) { 
...
// Process discontinued products ONLY
async function processDiscontinued() {
    ...
    try {
        ...
        const executeDiscontinue = async () => {
            ...
            // Process all products to discontinue (no artificial limit)
            for (let i = 0; i < toDiscontinue.length; i++) {
                ...
                // Log progress every 50 products
                if ((i + 1) % 50 === 0) {
                    addLog(`Progress: Discontinued ${i + 1}/${toDiscontinue.length} products...`, 'info');
                }
            }
...
app.get('/', (req, res) => {
    ...
    // Progress bars
    const productProgressHTML = syncProgress.fullImport.isActive && syncProgress.fullImport.total > 0 ? 
        `<div class="progress-container"><div class="progress-bar" style="width:${(syncProgress.fullImport.current / syncProgress.fullImport.total * 100).toFixed(1)}%"></div><small>${syncProgress.fullImport.current}/${syncProgress.fullImport.total} products</small></div>` : '';
    const discontinueProgressHTML = syncProgress.discontinue.isActive && syncProgress.discontinue.total > 0 ? 
        `<div class="progress-container"><div class="progress-bar" style="width:${(syncProgress.discontinue.current / syncProgress.discontinue.total * 100).toFixed(1)}%"></div><small>${syncProgress.discontinue.current}/${syncProgress.discontinue.total} products</small></div>` : '';
