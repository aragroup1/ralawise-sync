...
let AdmZip;
let ZIP_SUPPORT = false;
try {
    AdmZip = require('adm-zip');
    ZIP_SUPPORT = true;
    console.log('✅ ZIP support enabled');
} catch (e) {
    console.log('⚠️ ZIP support disabled. Install adm-zip to enable: npm install adm-zip');
}

const app = express();
app.use(express.json());

// Setup file upload - accepts ZIP files only if adm-zip is available
const upload = multer({ 
    dest: path.join(__dirname, 'uploads/'),
    limits: { fileSize: ZIP_SUPPORT ? 500 * 1024 * 1024 : 200 * 1024 * 1024 }, // 500MB for ZIP, 200MB for CSV
    fileFilter: (req, file, cb) => {
        const ext = path.extname(file.originalname).toLowerCase();
        if (ext === '.csv') {
            cb(null, true);
        } else if (ext === '.zip' && ZIP_SUPPORT) {
            cb(null, true);
        } else if (ext === '.zip' && !ZIP_SUPPORT) {
            cb(new Error('ZIP files not supported. Please install adm-zip or upload CSV files directly.'));
        } else {
            cb(new Error(`Only CSV ${ZIP_SUPPORT ? 'and ZIP ' : ''}files are allowed`));
        }
    }
});
...
if (requiredConfig.some(key => !process.env[key])) { 
    ...
    console.error('Required: ' + requiredConfig.join(', '));
    process.exit(1); 
}
...
if (ZIP_SUPPORT && !fs.existsSync(TEMP_EXTRACT_BASE)) fs.mkdirSync(TEMP_EXTRACT_BASE, { recursive: true });
...
function updateProgress(type, current, total) { 
    const p = syncProgress[type]; 
    if (!p) return; 
    p.current = current; 
    p.total = total; 
    if (p.startTime && current > 0) { 
        const e = Date.now() - p.startTime; 
        p.estimatedCompletion = new Date(Date.now() + (total - current) / (current / e)); 
    } 
}
...
// ============================================
// ZIP HANDLING FUNCTIONS (Only if adm-zip is available)
// ============================================

async function extractAndProcessZip(zipPath) {
    if (!ZIP_SUPPORT) {
        throw new Error('ZIP support not available. Please install adm-zip.');
    }
...
async function parseInventoryCSV(stream) { 
    return new Promise((resolve, reject) => { 
        ...
        stream
            .pipe(csv({ headers: ['SKU', 'Quantity'], skipLines: 1 }))
            .on('data', row => { 
                if (row.SKU) inventory.set(row.SKU.trim(), Math.min(parseInt(row.Quantity) || 0, config.ralawise.maxInventory)); 
            })
        ...
        if (changePercentage > config.failsafe.inventoryChangePercentage && updatesToPerform.length > 100) {
             requestConfirmation('inventory', 'High inventory change detected', { 
                 inventoryChange: { 
                     ...
                     updatesNeeded: updatesToPerform.length, 
                     sample: updatesToPerform.slice(0, 5).map(u => ({ sku: u.sku, oldQty: u.oldQty, newQty: u.newQty })) 
                 } 
             }, executeUpdates);
        } else {
...
async function processFullImport() {
    ...
    try {
        ...
        if (!catalogFiles?.productFile) {
            throw new Error('No product catalog file found. Please upload CSV files first.');
        }
        
        addLog(`Using product file: ${catalogFiles.productFile.name}`, 'info');
        ...
        // Parse main product catalog
        const allRows = await new Promise((res, rej) => { 
            const p = []; 
            fs.createReadStream(catalogFiles.productFile.path)
                .pipe(csv())
        ...
        for (const row of allRows) {
            ...
            if (discontinuedInfo?.shouldDiscontinue) {
                addLog(`Skipping discontinued SKU ${sku} (stock: ${discontinuedInfo.stock})`, 'info');
                continue;
            }
...
// ============================================
// SYNC WRAPPERS & API - WITH ZIP SUPPORT
// ============================================

async function syncInventory() { 
    if (isSystemLocked()) return; 
    try { 
        await updateInventoryBySKU(await parseInventoryCSV(await fetchInventoryFromFTP())); 
    } catch (error) { 
        triggerFailsafe(`Inventory sync failed: ${error.message}`); 
    } 
}
...
// File upload endpoint with ZIP support
    ...
    try {
        ...
        if (fs.existsSync(UPLOADED_FILES_DIR)) {
            fs.readdirSync(UPLOADED_FILES_DIR).forEach(file => {
                fs.unlinkSync(path.join(UPLOADED_FILES_DIR, file));
            });
        }
...
app.get('/api/upload/status', (req, res) => {
    const catalogFiles = getLatestCatalogFiles();
    res.json({
        hasFiles: !!catalogFiles?.productFile,
        productFile: catalogFiles?.productFile?.name,
        discontinuedFile: catalogFiles?.discontinuedFile?.name,
        uploadTime: catalogFiles?.productFile?.uploadTime,
...
app.post('/api/cleanup/once', (req, res) => { 
    cleanupProductsWithoutSupplierTags(); 
    res.json({ success: true }); 
});
...
app.post('/api/sync/inventory/cancel', (req, res) => { 
    if (isRunning.inventory) { 
        ...
        isRunning.inventory = false; 
        syncProgress.inventory.isActive = false; 
        addLog('User cancelled inventory sync.', 'warning'); 
    } 
    res.json({ success: true }); 
});
...
app.post('/api/confirmation/proceed', (req, res) => { 
    if (confirmation.isAwaiting) { 
        const action = confirmation.proceedAction; 
        resetConfirmationState(); 
        if (action) action(); 
    } 
    res.json({ success: true }); 
...
app.post('/api/confirmation/abort', (req, res) => { 
    if (confirmation.isAwaiting) { 
        confirmation.abortAction(); 
    } 
    res.json({ success: true }); 
});

app.get('/api/changes/latest', (req, res) => { 
    res.json(inventoryChangeLog); 
});
...
// ============================================
// WEB INTERFACE - WITH ZIP SUPPORT INDICATION
// ============================================

app.get('/', (req, res) => {
    const inventoryProgressHTML = syncProgress.inventory.isActive && syncProgress.inventory.total > 0 ? 
        `<div class="progress-container"><div class="progress-bar" style="width:${(syncProgress.inventory.current / syncProgress.inventory.total * 100).toFixed(1)}%"></div><small>${syncProgress.inventory.current}/${syncProgress.inventory.total} items</small></div>` : '';
    const cleanupProgressHTML = syncProgress.cleanup.isActive && syncProgress.cleanup.total > 0 ? 
        `<div class="progress-container"><div class="progress-bar" style="width:${(syncProgress.cleanup.current / syncProgress.cleanup.total * 100).toFixed(1)}%"></div><small>${syncProgress.cleanup.current}/${syncProgress.cleanup.total} products</small></div>` : '';
    ...
    const fileAcceptTypes = ZIP_SUPPORT ? '.csv,.zip' : '.csv';
    const fileUploadLabel = ZIP_SUPPORT ? '📁 Upload CSV/ZIP Files' : '📁 Upload CSV Files';
    const fileUploadHelp = ZIP_SUPPORT ? 
        'Accepts: CSV files or ZIP archives containing multiple CSV files' : 
        'Accepts: CSV files only (Install adm-zip for ZIP support)';

    const html = `<!DOCTYPE html><html lang="en"><head><title>Ralawise Sync</title><meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        ...
        .btn-warning{background:#d29922;color:white;border-color:#f0c674}
        ...
        .alert-warning{background-color:rgba(210,149,34,0.1);border-color:#d29922;}
        .stat-card{text-align:center;}
        .stat-value{font-size:2.5rem;font-weight:600;}
        .stat-label{font-size:0.8rem;color:#8b949e;}
        .product-list{list-style:none;padding:0;font-size:0.9em;max-height:150px;overflow-y:auto;} 
        .product-list a{color:#58a6ff;text-decoration:none;} 
        .product-list a:hover{text-decoration:underline;} 
        .progress-container{margin-top:0.5rem;} 
        .progress-bar{height:8px;background:#30363d;border-radius:4px;overflow:hidden; width: 100%;} 
        .progress-fill{height:100%;background:linear-gradient(90deg, #1f6feb, #2ea043);transition:width 0.5s;}
        .location-info{font-size:0.875em;color:#8b949e;margin-top:0.5rem;}
        .file-upload{margin-top:1rem;padding:1rem;border:2px dashed #30363d;border-radius:6px;text-align:center;}
        .file-upload input{display:none;}
        .file-upload label{cursor:pointer;padding:0.5rem 1rem;background:#21262d;border-radius:6px;display:inline-block;}
        .upload-status{margin-top:0.5rem;font-size:0.875em;color:#8b949e;}
    </style></head><body><div class="container"><h1>Ralawise Sync</h1>
    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <div class="file-upload">
                <label for="file-input" class="btn">${fileUploadLabel}</label>
                <input type="file" id="file-input" multiple accept="${fileAcceptTypes}" onchange="uploadFiles(this.files)">
                ...
                <div style="font-size:0.8em;color:#8b949e;margin-top:0.5rem;">${fileUploadHelp}</div>
            </div>
    ...
    async function uploadFiles(files){
        ...
        try{
            ...
            if(data.success){
                ...
                btn.textContent='${fileUploadLabel}';
                btn.style.opacity='1';
                progressDiv.innerHTML='';
            }
        }catch(e){
            alert('Upload error: '+e.message);
            document.querySelector('label[for="file-input"]').textContent='${fileUploadLabel}';
...
app.listen(PORT, () => {
    ...
    if (ZIP_SUPPORT) {
        addLog('✅ ZIP file support is ENABLED', 'success');
    } else {
        addLog('⚠️ ZIP file support is DISABLED. Install adm-zip to enable.', 'warning');
    }
    setTimeout(() => { 
        if (!isSystemLocked()) { 
            syncInventory(); 
