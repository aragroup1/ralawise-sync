...
const config = {
    ...
    failsafe: { 
        ...
        maxErrors: parseInt(process.env.MAX_ERRORS || '1000') // Increased due to many mismatches
    },
    rateLimit: {
...
async function updateInventoryBySKU(inventoryMap) { 
    ...
    try { 
        ...
        const productsNeedingTags = new Map(); // Changed to Map to avoid duplicates
        ...
        shopifyProducts.forEach(p => {
            ...
            p.variants?.forEach(v => {
                if (v.sku) {
                    ...
                    // Only add products needing tags if we have their SKU in inventory
        ...
        // Log sample of not found SKUs for debugging
        if (notFoundCount > 0) {
            const sampleNotFound = [];
            let count = 0;
            inventoryMap.forEach((qty, sku) => {
                if (!skuToProduct.has(sku.toUpperCase()) && count < 5) {
                    sampleNotFound.push(sku);
                    count++;
                }
            });
            addLog(`Sample of SKUs not found: ${sampleNotFound.join(', ')}`, 'warning');
        }
        ...
        const executeUpdates = async () => {
            ...
            // Step 3: Update inventory levels using REST API
            addLog(`Updating ${updatesToPerform.length} inventory levels using REST API...`, 'info');
            ...
            for (let i = 0; i < updatesToPerform.length; i += BATCH_SIZE) {
                ...
                for (const u of batch) {
                    ...
                    try {
                        ...
                        // If set fails, try connect first
                        if (e.response && e.response.status === 422) {
                            try {
                                ...
                                syncProgress.inventory.errors++;
                                runResult.errors++;
                                addLog(`Failed to update ${u.sku}: ${retryError.message}`, 'error');
                            }
                        } else {
                            syncProgress.inventory.errors++;
                            runResult.errors++;
                ...
                // Update progress
                updateProgress('inventory', syncProgress.inventory.updated, updatesToPerform.length);
                
                // Log progress every 100 items or 10%
                const progressPercent = (syncProgress.inventory.updated / updatesToPerform.length) * 100;
                if (syncProgress.inventory.updated % 100 === 0 || progressPercent % 10 < 1) {
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert { padding: 1.5rem; border-radius: 16px; margin-bottom: 2rem; border: 1px solid; }
...
<body>
    <div class="container">
        ...
        ${failsafe.isTriggered ? 
            `<div class="alert failsafe">
                ...
                <button onclick="clearFailsafe()" class="btn btn-primary">Clear Failsafe</button>
            </div>` : ''}
        ...
        ${confirmation.isAwaiting ? 
            `<div class="alert confirmation">
                ...
                <button onclick="abort()" class="btn btn-danger">Abort</button>
            </div>` : ''}
    ...
    <script>
        ...
        function clearFailsafe() { apiPost('/api/failsafe/clear'); }
        function proceed() { apiPost('/api/confirmation/proceed'); }
        function abort() { apiPost('/api/confirmation/abort'); }
