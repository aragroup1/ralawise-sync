...
// --- NEW: Smart Shopify Client with Retry Logic ---
...
async function shopifyRequestWithRetry(method, url, data = null, retries = 3) {
    ...
    while (attempt < retries) {
        try {
            ...
            if (error.response && error.response.status === 429) {
                ...
                addLog(`Shopify rate limit hit. Retrying in ${waitTime / 1000}s... (Attempt ${attempt}/${retries})`, 'warning');
    ...
    throw new Error(`Shopify API request failed after ${retries} retries.`);
}
...
async function getAllShopifyProducts() { let all = []; let url = `/products.json?limit=250&fields=id,handle,title,variants,tags,status`; while (url) { const res = await shopifyRequestWithRetry('get', url); all.push(...res.data.products); const link = res.headers.link; url = link && link.includes('rel="next"') ? link.match(/<([^>]+)>/)[1].replace(config.shopify.baseUrl, '') : null; await delay(250); } addLog(`Fetched ${all.length} products from Shopify`, 'success'); return all; }
...
async function updateInventoryBySKU(inventoryMap) {
    ...
    let runResult = { type: 'Inventory', timestamp: new Date().toISOString(), status: 'failed', updated: 0, tagged: 0, errors: 0 };
    try {
        ...
        inventoryMap.forEach((newQty, sku) => {
            ...
            if (match && (match.variant.inventory_quantity || 0) !== newQty) updatesToPerform.push({ sku, oldQty: match.variant.inventory_quantity || 0, newQty, match });
        });
        ...
        const executeUpdates = async () => {
            ...
            for (const u of updatesToPerform) {
                try {
                    if (!u.match.product.tags?.includes('Supplier:Ralawise')) { await shopifyRequestWithRetry('put', `/products/${u.match.product.id}.json`, { product: { id: u.match.product.id, tags: `${u.match.product.tags || ''},Supplier:Ralawise`.replace(/^,/, '') } }); runResult.tagged++; await delay(200); }
                    if (!u.match.variant.inventory_management) { await shopifyRequestWithRetry('put', `/variants/${u.match.variant.id}.json`, { variant: { id: u.match.variant.id, inventory_management: 'shopify', inventory_policy: 'deny' } }); await delay(200); }
                    await shopifyRequestWithRetry('post', '/inventory_levels/connect.json', { location_id: config.shopify.locationId, inventory_item_id: u.match.variant.inventory_item_id }).catch(() => {});
                    ...
                    runResult.errors++;
                    addLog(`Failed to update ${u.sku}: ${e.message}`, 'error');
                }
            }
            ...
            notifyTelegram(`Inventory update complete:\n✅ ${runResult.updated} updated, 🏷️ ${runResult.tagged} tagged, ❌ ${runResult.errors} errors, ❓ ${notFound} not found`);
        };

        if (changePercentage > config.failsafe.inventoryChangePercentage) {
            requestConfirmation('inventory', `High inventory change detected`, { inventoryChange: { threshold: config.failsafe.inventoryChangePercentage, actualPercentage: changePercentage, updatesNeeded, totalProducts, sample: updatesToPerform.slice(0, 10).map(u => ({ sku: u.sku, oldQty: u.oldQty, newQty: u.newQty })) }}, async () => {
                try { await executeUpdates(); } finally { isRunning.inventory = false; }
            });
            return;
        } else {
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            :root { --bg-color: #0a0a0a; --card-bg: rgba(255, 255, 255, 0.05); --text-color: #EAEAEA; --text-muted: #A0A0A0; --border-color: rgba(255, 215, 0, 0.2); --accent-gold: #FFD700; --primary-gradient: linear-gradient(90deg, #8E2DE2 0%, #4A00E0 100%); --green-gradient: linear-gradient(90deg, #11998e 0%, #38ef7d 100%); --amber-gradient: linear-gradient(90deg, #FFC107 0%, #FF8F00 100%); --red-gradient: linear-gradient(90deg, #cb2d3e 0%, #ef473a 100%); }
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Inter', sans-serif; background-color: var(--bg-color); color: var(--text-color); min-height: 100vh; padding: 2rem; background-image: radial-gradient(circle at top right, rgba(74, 0, 224, 0.1) 0%, transparent 40%), radial-gradient(circle at bottom left, rgba(142, 45, 226, 0.1) 0%, transparent 50%); }
            .container { max-width: 1400px; margin: 0 auto; display: grid; gap: 2rem; }
            .header { text-align: center; margin-bottom: 2rem; } .header h1 { font-family: 'Playfair Display', serif; font-size: 3rem; font-weight: 700; background: linear-gradient(45deg, var(--accent-gold), #fff, var(--accent-gold)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; } .header p { color: var(--text-muted); font-size: 1.1rem; }
            .card { background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 16px; padding: 2rem; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); box-shadow: 0 8px 32px rgba(0,0,0,0.2); transition: transform 0.3s ease, box-shadow 0.3s ease; }
            .card:hover { transform: translateY(-5px); box-shadow: 0 12px 40px rgba(0,0,0,0.3); }
            .card h2 { margin-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; font-weight: 600; font-family: 'Playfair Display', serif; }
            ...
            .stat h3 { font-size: 0.9rem; color: var(--text-muted); margin-bottom: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; } .stat p { font-size: 2.5rem; font-weight: 700; line-height: 1; color: var(--accent-gold); } .stat small { display: block; margin-top: 0.75rem; color: var(--text-muted); font-size: 0.8rem; }
            ...
            .logs, .history-table { background: rgba(0,0,0,0.3); padding: 1.5rem; border-radius: 12px; max-height: 400px; overflow-y: auto; font-family: 'SF Mono', monospace; font-size: 0.85rem; line-height: 1.6; }
            ...
            .banner { padding: 2rem; border-radius: 16px; margin-bottom: 2rem; border: 1px solid; } .banner h2 { border-bottom: 1px solid; padding-bottom: 1rem; margin-bottom: 1rem; font-family: 'Playfair Display', serif; }
            ...
            table { width: 100%; border-collapse: collapse; } th, td { padding: 0.75rem 1rem; text-align: left; border-bottom: 1px solid var(--border-color); } thead { background: rgba(255, 255, 255, 0.05); } th { color: var(--accent-gold); }
        </style>
    </head>
    <body>
        <div class="container">
            <header class="header"><h1>Ralawise Sync</h1><p>Automated Inventory & Product Management</p></header>
            ...
            <div class="card">
              ...
              <div class="btn-group">
                <button onclick="togglePause()" class="${isSystemPaused ? 'btn-resume' : 'btn-pause'}" ${failsafe.isTriggered || confirmation.isAwaiting ? 'disabled' : ''}>${isSystemPaused ? '▶️ Resume System' : '⏸️ Pause System'}</button>
            ...
            <div class="card">
                ...
                <div class="history-table">
                    <table>
                        <thead><tr><th>Timestamp</th><th>Type</th><th>Status</th><th>Details</th></tr></thead>
                        <tbody>
                            ${runHistory.map(run => `
                                <tr>
                                    <td>${new Date(run.timestamp).toLocaleString()}</td>
                                    <td>${run.type}</td>
                                    <td style="color:${run.status === 'completed' ? '#56d364' : '#f85149'}; text-transform: capitalize;">${run.status}</td>
                                    <td>
                                        ${run.type === 'Inventory' ? `${run.updated || 0} updated, ${run.tagged || 0} tagged` : ''}
                                        ${run.type === 'Full Import' ? `${run.created || 0} created, ${run.discontinued || 0} discontinued` : ''}
                                        ${run.errors > 0 ? `, ${run.errors} errors` : ''}
                                    </td>
                                </tr>
                            `).join('')}
                            ${runHistory.length === 0 ? `<tr><td colspan="4" style="text-align:center; color: var(--text-muted);">No runs have been recorded yet.</td></tr>` : ''}
                        </tbody>
                    </table>
                </div>
            </div>
