...
async function shopifyRequestWithRetry(method, url, data = null, retries = 3) { 
    ...
    while (attempt < retries) { 
        try { 
            ...
            if (error.response && error.response.status === 429) { 
                ...
                addLog(`Shopify rate limit hit. Retrying in ${(retryAfter / 1000).toFixed(1)}s... (Attempt ${attempt}/${retries})`, 'warning'); 
    ...
    throw new Error(`Shopify API request for ${method.toUpperCase()} ${url} failed after ${retries} retries.`); 
}
...
// ENHANCED: Inventory update with progress tracking and cancellation
async function updateInventoryBySKU(inventoryMap) { 
    ...
    try { 
        ...
        const executeUpdates = async () => {
            ...
            // Process in batches for better performance and control
            const BATCH_SIZE = 10;
            const batches = [];
            for (let i = 0; i < updatesToPerform.length; i += BATCH_SIZE) {
                batches.push(updatesToPerform.slice(i, i + BATCH_SIZE));
            }
            
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                // Check cancellation
                if (syncProgress.inventory.cancelled) {
                    addLog('Inventory sync cancelled', 'warning');
                    break;
                }
                ...
                for (const u of batch) {
                    ...
                    try {
                        // Tag product if needed
                        if (!u.match.product.tags?.includes('Supplier:Ralawise')) {
                            await shopifyRequestWithRetry('put', `/products/${u.match.product.id}.json`, {
                                product: {
                                    id: u.match.product.id,
                                    tags: `${u.match.product.tags || ''},Supplier:Ralawise`.replace(/^,/, '')
                                }
                            });
                        ...
                        if (!u.match.variant.inventory_management) {
                            await shopifyRequestWithRetry('put', `/variants/${u.match.variant.id}.json`, {
                                variant: {
                                    id: u.match.variant.id,
                        ...
                        if (syncProgress.inventory.updated % 10 === 0) {
                            ...
                            const progressMsg = `Progress: ${syncProgress.inventory.updated}/${syncProgress.inventory.total} (${percent}%) - ETA: ${eta}`;
                            addLog(progressMsg, 'info');
                            
                            // Telegram update every 500 items
                            if (syncProgress.inventory.updated % 500 === 0) {
                        ...
                        if (syncProgress.inventory.errors > config.failsafe.maxErrors) {
                            addLog(`Too many errors (>${config.failsafe.maxErrors}), stopping inventory sync`, 'error');
                            syncProgress.inventory.cancelled = true;
                            break;
                        }
                    }
                ...
                if (batchIndex < batches.length - 1 && !syncProgress.inventory.cancelled) {
                    await delay(1000);
                }
            }
...
async function processFullImport(csvFiles) { 
    ...
    try { 
        ...
        // Create new products (limited to prevent overwhelming)
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status.paused { background: rgba(245, 158, 11, 0.1); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.3); } 
        .status.running { background: rgba(59, 130, 246, 0.1); color: #3b82f6; border: 1px solid rgba(59, 130, 246, 0.3); animation: pulse 2s infinite; }
        ...
        .btn-secondary { background: var(--gradient-dark); color: var(--text-primary); border: 1px solid rgba(255, 255, 255, 0.1); }
        ...
        .alert.failsafe::before { background: var(--gradient-danger); }
        
        .alert.confirmation { background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.3); } 
        .alert.confirmation::before { background: var(--gradient-primary); }
        ...
        .history-table thead { background: rgba(255, 255, 255, 0.02); } 
        .history-table th { 
            font-size: 0.75rem; 
            text-transform: uppercase; 
            letter-spacing: 1px; 
        ...
        <div class="grid grid-3">
            <div class="card">
                ...
                <div style="margin-top: 1rem;">
                    <button onclick="togglePause()" class="btn ${isSystemPaused ? 'btn-success' : 'btn-warning'}" ${!canTogglePause ? 'disabled' : ''}>
            ...
            <div class="card">
                ...
                <div class="btn-group" style="flex-direction: column;">
                    ...
                    <button onclick="clearLogs()" class="btn btn-secondary" ${isSystemLockedForActions ? 'disabled' : ''}>
                        🗑️ Clear Logs
                    </button>
                    ${isRunning.inventory ? 
                        `<button onclick="cancelInventory()" class="btn btn-danger">
                            🛑 Cancel Inventory Sync
                        </button>` : ''}
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            ${runHistory.slice(0, 10).map(run => 
                                `<tr>
                                    ...
                                    <td><span class="status ${run.type.toLowerCase().includes('inventory') ? 'info' : 'active'}">${run.type}</span></td>
                                    <td>${run.status || 'completed'}</td>
                                    <td>${run.updated ?? '-'}</td>
                                    <td>${run.created ?? '-'}</td>
                                    <td>${run.tagged ?? '-'}</td>
                                    <td>${run.discontinued ?? '-'}</td>
                                    <td style="color:${(run.errors || 0) > 0 ? 'var(--accent-danger)' : 'inherit'}">${run.errors || 0}</td>
                                </tr>`
                            ).join('')}
                        </tbody>
                    </table>
        ...
        <div class="card" style="animation-delay: 0.3s;">
            ...
            <div class="logs" id="logs">
                ${logs.length > 0 ? 
                    ...
                    '<div class="log-entry log-info">System initialized. Waiting for operations...</div>'}
            </div>
        </div>
    </div>
...
// NEW: Cancel inventory sync endpoint
app.post('/api/sync/inventory/cancel', (req, res) => {
    ...
    res.json({ success: true, message: 'Cancellation requested. The sync will stop after the current item.' });
});

// NEW: Get progress endpoint
...
// Daily inventory sync at 2:00 AM
cron.schedule('0 2 * * *', () => {
...
// Full catalog sync every 2 days at 1:00 PM
