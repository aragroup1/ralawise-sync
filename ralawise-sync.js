...
// --- MODIFIED: Added detailed logging to getAllShopifyProducts ---
async function getAllShopifyProducts() { 
    ...
    while (url) { 
        ...
        url = link && link.includes('rel="next"') ? link.match(/<([^>]+)>/)[1].replace(config.shopify.baseUrl, '') : null; 
        await delay(250); // Be polite to the API
    } 
    addLog(`Completed fetching ${all.length} products from Shopify`, 'success'); 
    return all; 
}
