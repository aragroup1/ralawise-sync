...
const PRODUCTS_FILE = path.join(__dirname, '_products_tracking.json');
...
function loadProductsTracking() {
    try {
        if (fs.existsSync(PRODUCTS_FILE)) {
            productsTracking = JSON.parse(fs.readFileSync(PRODUCTS_FILE, 'utf8'));
            // Clean old entries (older than 30 days)
            const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
            productsTracking.created = productsTracking.created.filter(p => new Date(p.timestamp) > thirtyDaysAgo);
            productsTracking.discontinued = productsTracking.discontinued.filter(p => new Date(p.timestamp) > thirtyDaysAgo);
            addLog(`Loaded product tracking: ${productsTracking.created.length} created, ${productsTracking.discontinued.length} discontinued.`, 'info');
        }
    } catch (e) {
        addLog(`Could not load product tracking: ${e.message}`, 'warning');
    }
}

function saveProductsTracking() {
    try {
        // Keep only last 1000 of each
        if (productsTracking.created.length > 1000) {
            productsTracking.created = productsTracking.created.slice(-1000);
        }
        if (productsTracking.discontinued.length > 1000) {
            productsTracking.discontinued = productsTracking.discontinued.slice(-1000);
        }
        productsTracking.lastUpdated = new Date().toISOString();
        fs.writeFileSync(PRODUCTS_FILE, JSON.stringify(productsTracking, null, 2));
    } catch (e) {
        addLog(`Could not save product tracking: ${e.message}`, 'warning');
    }
}

function trackProductCreated(product) {
    const record = {
    ...
    productsTracking.created.push(record);
    syncProgress.fullImport.newProducts.push(record);
}

function trackProductDiscontinued(product) {
    const record = {
        timestamp: new Date().toISOString(),
        title: product.title,
        handle: product.handle,
    ...
    productsTracking.discontinued.push(record);
}
...
async function processFullImport(csvFiles) { 
    ...
    try { 
        ...
        for (const p of toDiscontinue.slice(0, 100)) {
            if (syncProgress.fullImport.cancelled) break;
        ...
        saveProductsTracking();
        
        runResult.status = syncProgress.fullImport.cancelled ? 'cancelled' : 'completed';
        
        // Send detailed notification with new products
        let notificationMsg = `Full import ${runResult.status}:\n✅ ${runResult.created} created\n⏸️ ${runResult.discontinued} discontinued\n❌ ${runResult.errors} errors`;
        
        if (syncProgress.fullImport.newProducts.length > 0) {
            notificationMsg += `\n\n🆕 New Products Created:\n`;
            syncProgress.fullImport.newProducts.slice(0, 5).forEach(p => {
                notificationMsg += `• ${p.title} (${p.sku})\n`;
            });
            if (syncProgress.fullImport.newProducts.length > 5) {
                notificationMsg += `... and ${syncProgress.fullImport.newProducts.length - 5} more`;
            }
        }
        
        notifyTelegram(notificationMsg);
...
app.get('/', (req, res) => {
    ...
    const totalCreated = runHistory.reduce((sum, r) => sum + (r.created || 0), 0);
    const totalDiscontinued = runHistory.reduce((sum, r) => sum + (r.discontinued || 0), 0);
    const recentCreated = last7d.reduce((sum, r) => sum + (r.created || 0), 0);
    const recentDiscontinued = last7d.reduce((sum, r) => sum + (r.discontinued || 0), 0);
    
    // Get recently created products
    const recentlyCreatedProducts = productsTracking.created.slice(-10);
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .progress-meta { display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem; font-size: 0.875rem; }
        ...
        .alert { padding: 1.5rem; border-radius: 16px; margin-bottom: 2rem; border: 1px solid; background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.3); }
        .alert.failsafe { background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.3); }
        .alert.confirmation { background: rgba(251, 191, 36, 0.1); border-color: rgba(251, 191, 36, 0.3); }
        .alert h3 { font-size: 1.25rem; font-weight: 600; margin-bottom: 0.75rem; }
        .alert p { color: var(--text-secondary); margin-bottom: 1rem; }
        .confirmation-details { margin-top: 1rem; }
        .detail-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-bottom: 1rem; }
        .detail-card { background: rgba(0, 0, 0, 0.3); padding: 1rem; border-radius: 12px; text-align: center; }
        .detail-label { display: block; font-size: 0.75rem; color: var(--text-muted); margin-bottom: 0.5rem; }
        .detail-value { display: block; font-size: 1.5rem; font-weight: 700; }
        .detail-value.danger { color: var(--accent-danger); }
        .sample-changes { background: rgba(0, 0, 0, 0.3); padding: 1rem; border-radius: 12px; margin-top: 1rem; }
        .sample-changes h4 { font-size: 0.875rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-secondary); }
        .changes-list { display: grid; gap: 0.5rem; }
        .change-item { display: flex; align-items: center; gap: 1rem; padding: 0.5rem; background: rgba(255, 255, 255, 0.02); border-radius: 8px; font-family: monospace; font-size: 0.875rem; }
        .change-item .sku { color: var(--accent-primary); font-weight: 600; min-width: 100px; }
        .change-item .arrow { color: var(--text-muted); }
        .change-item .qty-change { color: var(--text-primary); }
        .product-stats { background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3); }
        .product-list { background: rgba(0, 0, 0, 0.3); padding: 1rem; border-radius: 12px; margin-top: 1rem; max-height: 200px; overflow-y: auto; }
        .product-item { padding: 0.5rem; margin-bottom: 0.5rem; background: rgba(255, 255, 255, 0.02); border-radius: 8px; display: flex; justify-content: space-between; align-items: center; }
        .product-item .title { color: var(--text-primary); font-weight: 500; }
        .product-item .date { color: var(--text-muted); font-size: 0.75rem; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="card product-stats">
            ...
            <div class="grid grid-4">
                <div class="stat-card">
                    <div class="label">Products Created (7d)</div>
                    <div class="value">${recentCreated}</div>
                </div>
                <div class="stat-card">
                    <div class="label">Products Discontinued (7d)</div>
                    <div class="value">${recentDiscontinued}</div>
                </div>
                <div class="stat-card">
                ...
                <div class="stat-card">
                    <div class="label">Total Discontinued</div>
                    <div class="value">${totalDiscontinued}</div>
                </div>
            </div>
            ${recentlyCreatedProducts.length > 0 ? `
            <div class="product-list">
                <h3 style="margin-bottom: 1rem;">🆕 Recently Created Products</h3>
                ${recentlyCreatedProducts.map(p => `
                    <div class="product-item">
                        <div>
                            <div class="title">${p.title}</div>
                            <small style="color: var(--text-muted);">SKU: ${p.sku || 'N/A'}</small>
                        </div>
        ...
        <div class="grid grid-4">
            ...
            <div class="card">
                ...
                <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.5rem;">Creates new products & discontinues old ones</p>
    ...
    <script>
        ...
        function runFullImport() { apiPost('/api/sync/full', 'Run full import? This will create new products and mark discontinued ones as draft.'); }
...
app.post('/api/failsafe/clear', (req, res) => {
    ...
    isRunning.inventory = false;
    isRunning.fullImport = false;
    notifyTelegram('✅ Failsafe has been manually cleared.');
...
app.post('/api/confirmation/abort', (req, res) => {
    ...
    res.json({ success: true, message: 'Action aborted.' });
});
...
app.get('/api/products/recent', (req, res) => {
    res.json({ 
        success: true, 
        created: productsTracking.created.slice(-50),
        discontinued: productsTracking.discontinued.slice(-50)
    });
});
