...
async function shopifyGraphQLRequest(query, variables, retries = 3) {
    ...
    while (attempt < retries) {
        try {
            const response = await shopifyClient.post('/graphql.json', { query, variables });
            if (response.data.errors) {
                // Handle GraphQL-specific errors, which come with a 200 status
                const errorMessages = response.data.errors.map(e => e.message).join(', ');
                if (errorMessages.includes('Throttled')) {
                     // This is a rate limit error in GraphQL
                    throw { response: { status: 429, headers: {} }, message: 'GraphQL Throttled' };
                }
            ...
            if (error.response && error.response.status === 429) {
                ...
                addLog(`Shopify GraphQL rate limit hit. Retrying in ${(retryAfter / 1000).toFixed(1)}s... (Attempt ${attempt}/${retries})`, 'warning');
...
async function updateInventoryBySKU(inventoryMap) {
    ...
    let runResult = { type: 'Inventory', status: 'failed', updated: 0, tagged: 0, configured: 0, errors: 0 };

    try {
        addLog('=== STARTING EFFICIENT INVENTORY SYNC ===', 'info');
        ...
        // 2. Analyze differences and categorize tasks
        const inventoryAdjustments = [];
        const oneTimeSetupTasks = [];
        ...
        inventoryMap.forEach((newQty, sku) => {
            ...
            if (match) {
                ...
                // Task: Inventory Level Adjustment (if quantity differs)
                if (oldQty !== newQty) {
                    if (!variant.inventory_item_id) {
                        addLog(`Skipping ${sku}: Missing inventory_item_id.`, 'warning');
                        runResult.errors++;
                        return;
                    }
                    inventoryAdjustments.push({
                        inventoryItemId: `gid://shopify/InventoryItem/${variant.inventory_item_id}`,
                        availableDelta: newQty - oldQty, // Bulk update uses a delta (the change)
                        sku: sku,
                        title: product.title
                    });
                }
                
                // Task: One-time setup (tagging, inventory management)
                if (!product.tags?.includes('Supplier:Ralawise') || variant.inventory_management !== 'shopify') {
                    oneTimeSetupTasks.push({ product, variant });
                }
            }
        });

        const totalProducts = skuToProduct.size;
        const updatesNeeded = inventoryAdjustments.length;
        const changePercentage = totalProducts > 0 ? (updatesNeeded / totalProducts) * 100 : 0;
        addLog(`Change analysis: ${updatesNeeded} inventory updates and ${oneTimeSetupTasks.length} setup tasks for ${totalProducts} products (${changePercentage.toFixed(2)}%).`, 'info');

        const executeUpdates = async () => {
            // --- Stage 1: Perform one-time setup updates (still one-by-one, but less frequent) ---
            if (oneTimeSetupTasks.length > 0) {
                addLog(`Performing ${oneTimeSetupTasks.length} one-time setup tasks...`, 'info');
                for (const task of oneTimeSetupTasks) {
                    try {
                        // Add tag if missing
                        if (!task.product.tags?.includes('Supplier:Ralawise')) {
                            await shopifyRequestWithRetry('put', `/products/${task.product.id}.json`, {
                                product: { id: task.product.id, tags: `${task.product.tags || ''},Supplier:Ralawise`.replace(/^,/, '') }
                            });
                            runResult.tagged++;
                            addLog(`Tagged: ${task.product.title}`, 'info');
                            await delay(250);
                        }
                        // Set inventory management if not set
                        if (task.variant.inventory_management !== 'shopify') {
                            await shopifyRequestWithRetry('put', `/variants/${task.variant.id}.json`, {
                                variant: { id: task.variant.id, inventory_management: 'shopify', inventory_policy: 'deny' }
                            });
                             await shopifyRequestWithRetry('post', '/inventory_levels/connect.json', { location_id: config.shopify.locationId, inventory_item_id: task.variant.inventory_item_id }).catch(() => {});
                            runResult.configured++;
                            addLog(`Configured inventory tracking for: ${task.product.title} (${task.variant.sku})`, 'info');
                            await delay(250);
                        }
                    } catch(e) {
                        runResult.errors++;
                        addLog(`Failed setup for ${task.product.title}: ${e.message}`, 'error');
                    }
                }
            }

            // --- Stage 2: Perform bulk inventory update using GraphQL ---
            if (inventoryAdjustments.length > 0) {
                addLog(`Executing ${inventoryAdjustments.length} inventory updates in batches...`, 'info');
                const BATCH_SIZE = 100; // Shopify's limit for this mutation
                const mutation = `
                ...
                for (let i = 0; i < inventoryAdjustments.length; i += BATCH_SIZE) {
                    const batch = inventoryAdjustments.slice(i, i + BATCH_SIZE);
                    const batchItems = batch.map(item => ({ inventoryItemId: item.inventoryItemId, availableDelta: item.availableDelta }));
                    const variables = {
                        locationId: `gid://shopify/Location/${config.shopify.locationId}`,
                        inventoryItemAdjustments: batchItems
                    };

                    try {
                        addLog(`Submitting batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(inventoryAdjustments.length / BATCH_SIZE)} (${batch.length} items)...`, 'info');
                        const result = await shopifyGraphQLRequest(mutation, variables);

                        if (result.data.inventoryBulkAdjustQuantityAtLocation.userErrors.length > 0) {
                            result.data.inventoryBulkAdjustQuantityAtLocation.userErrors.forEach(err => {
                                addLog(`Batch Error for SKU in fields ${err.field.join(', ')}: ${err.message}`, 'error');
                                runResult.errors++;
                            });
                        } else {
                            runResult.updated += batch.length;
                            addLog(`Batch ${Math.floor(i / BATCH_SIZE) + 1} completed successfully. Total updated: ${runResult.updated}`, 'success');
                        }
                    } catch (e) {
                        runResult.errors += batch.length; // Assume whole batch failed on critical error
                        addLog(`Failed to submit batch ${Math.floor(i / BATCH_SIZE) + 1}: ${e.message}`, 'error');
                    }
                    await delay(500); // A small delay between batches is good practice
                }
            }
            
            runResult.status = 'completed';
            notifyTelegram(`Inventory update complete:\n✅ ${runResult.updated} updated\n🏷️ ${runResult.tagged} tagged\n🛠️ ${runResult.configured} configured\n❌ ${runResult.errors} errors`);
        };
        ...
        if (changePercentage > config.failsafe.inventoryChangePercentage) {
            requestConfirmation('inventory', 'High inventory change detected', {
                inventoryChange: {
                    ...
                    updatesNeeded,
                    totalProducts,
                    sample: inventoryAdjustments.slice(0, 10).map(u => ({ sku: u.sku, change: u.availableDelta }))
                }
            }, async () => {
                try {
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .change-item .sku { color: var(--accent-primary); font-weight: 600; flex-grow: 1; } .change-item .qty-change { font-weight: 600; } .qty-change.positive { color: var(--accent-success); } .qty-change.negative { color: var(--accent-warning); }
...
<body>
    <div class="container">
        ...
        <div class="grid grid-3">
            <div class="card"><h2>System Status</h2><div class="stat-card"><div class="label">Overall Status</div><div class="value"><span class="status ${isSystemPaused ? 'paused' : 'active'}">${isSystemPaused ? '⏸️ Paused' : '✅ Active'}</span></div></div><div style="margin-top: 1rem;"><button onclick="togglePause()" class="btn ${isSystemPaused ? 'btn-success' : 'btn-warning'}" ${!canTogglePause ? 'disabled' : ''}>${isSystemPaused ? '▶️ Resume System' : '⏸️ Pause System'}</button></div></div>
            <div class="card"><h2>Job Status</h2><div style="display: grid; gap: 1rem;"><div class="stat-card"><div class="label">Inventory Sync</div><div class="value"><span class="status ${isRunning.inventory ? 'running' : ''}">${isRunning.inventory ? '🔄 Running' : '✅ Ready'}</span></div></div><div class="stat-card"><div class="label">Full Import</div><div class="value"><span class="status ${isRunning.fullImport ? 'running' : ''}">${isRunning.fullImport ? '🔄 Running' : '✅ Ready'}</span></div></div></div></div>
