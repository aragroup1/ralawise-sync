...
let runHistory = [];
let syncProgress = { inventory: { isActive: false }, fullImport: { isActive: false } };

class RateLimiter {
    constructor(requestsPerSecond = 2, burstSize = 40) {
        this.requestsPerSecond = requestsPerSecond; this.burstSize = burstSize; this.tokens = burstSize;
        this.lastRefill = Date.now(); this.queue = []; this.processing = false;
    }
    async acquire() { return new Promise(resolve => { this.queue.push(resolve); this.process(); }); }
    async process() {
        if (this.processing) return; this.processing = true;
        while (this.queue.length > 0) {
            const now = Date.now(); const timePassed = (now - this.lastRefill) / 1000;
            this.tokens = Math.min(this.burstSize, this.tokens + timePassed * this.requestsPerSecond);
            this.lastRefill = now;
            if (this.tokens >= 1) { this.tokens--; const resolve = this.queue.shift(); resolve(); } 
            else { const waitTime = (1 / this.requestsPerSecond) * 1000; await new Promise(r => setTimeout(r, waitTime)); }
        }
...
async function updateInventoryBySKU(inventoryMap) {
    if (isRunning.inventory) { addLog('Inventory sync already running.', 'warning'); return; }
    isRunning.inventory = true;

    syncProgress.inventory = { isActive: true, current: 0, total: 0, startTime: Date.now() };
    let runResult = { type: 'Inventory', status: 'failed', updated: 0, skipped: 0, errors: 0, notFound: 0 };
    
    const finalizeRun = () => { isRunning.inventory = false; syncProgress.inventory.isActive = false; addToHistory({ ...runResult, timestamp: new Date().toISOString() }); };
    ...
    try {
        ...
        const executeUpdates = async () => {
            for (let i = 0; i < updatesToPerform.length; i++) {
                ...
                try {
                    ...
                    runResult.errors++;
                    if (e.response?.status === 422) { try { await shopifyRequestWithRetry('post', '/inventory_levels/connect.json', { location_id: config.shopify.locationId, inventory_item_id: u.match.variant.inventory_item_id }); } catch (connectError) { /* ignore */ } }
                }
        ...
        if (changePercentage > config.failsafe.inventoryChangePercentage && updatesToPerform.length > 100) {
             requestConfirmation('inventory', 'High inventory change detected', { inventoryChange: { threshold: config.failsafe.inventoryChangePercentage, actualPercentage: changePercentage, updatesNeeded: updatesToPerform.length, sample: updatesToPerform.slice(0, 5).map(u => ({ sku: u.sku, oldQty: u.oldQty, newQty: u.newQty })) } }, 
                async () => { try { await executeUpdates(); } finally { finalizeRun(); } }
             );
             return;
        } else { await executeUpdates(); finalizeRun(); }
    } catch (error) { triggerFailsafe(`Inventory sync failed: ${error.message}`); finalizeRun(); }
}
...
async function syncInventory() { if (isSystemLocked()) return; try { isRunning.inventory = true; addLog('=== INVENTORY SYNC TRIGGERED ===', 'info'); const stream = await fetchInventoryFromFTP(); await updateInventoryBySKU(await parseInventoryCSV(stream)); } catch (error) { triggerFailsafe(`Inventory sync failed: ${error.message}`); } }
async function syncFullCatalog() { if (isSystemLocked()) return; let tempDir; try { isRunning.fullImport = true; addLog('=== FULL CATALOG SYNC TRIGGERED ===', 'info'); const { tempDir: dir, csvFiles } = await downloadAndExtractZip(); tempDir = dir; await processFullImport(csvFiles); } catch (error) { triggerFailsafe(`Full catalog sync failed: ${error.message}`); } finally { if (tempDir) fs.rmSync(tempDir, { recursive: true, force: true }); } }
...
app.post('/api/sync/inventory/cancel', (req, res) => { if (isRunning.inventory) { syncProgress.inventory.cancelled = true; } res.json({ success: true }); });
...
app.get('/', (req, res) => {
    ...
    </div><script>async function apiPost(url,confirmMsg){if(confirmMsg&&!confirm(confirmMsg))return;try{const btn=event.target;btn.disabled=true;await fetch(url,{method:'POST'});setTimeout(()=>location.reload(),500)}catch(e){alert(e.message)}};setTimeout(()=>location.reload(),60000)</script></body></html>`;
    res.send(html);
});
