...
async function syncInventory() { if (isSystemPaused || failsafe.isTriggered || confirmation.isAwaiting) { addLog(`Sync skipped: System is ${isSystemPaused ? 'PAUSED' : failsafe.isTriggered ? 'in FAILSAFE' : 'awaiting confirmation'}.`, 'warning'); return; } try { addLog('=== INVENTORY SYNC TRIGGERED ===', 'info'); const stream = await fetchInventoryFromFTP(); await updateInventoryBySKU(await parseInventoryCSV(stream)); } catch (error) { triggerFailsafe(`Inventory sync failed: ${error.message}`); addToHistory({ type: 'Inventory', timestamp: new Date().toISOString(), status: 'failed', errors: 1 }); } }
async function syncFullCatalog() { if (isSystemPaused || failsafe.isTriggered || confirmation.isAwaiting) { addLog(`Sync skipped: System is ${isSystemPaused ? 'PAUSED' : failsafe.isTriggered ? 'in FAILSAFE' : 'awaiting confirmation'}.`, 'warning'); return; } let tempDir; try { addLog('=== FULL CATALOG SYNC TRIGGERED ===', 'info'); const { tempDir: dir, csvFiles } = await downloadAndExtractZip(); tempDir = dir; await processFullImport(csvFiles); } catch (error) { triggerFailsafe(`Full catalog sync failed: ${error.message}`); addToHistory({ type: 'Full Import', timestamp: new Date().toISOString(), status: 'failed', errors: 1 }); } finally { if (tempDir) { try { fs.rmSync(tempDir, { recursive: true, force: true }); addLog('Cleaned up temp files', 'info'); } catch (cleanupError) { addLog(`Cleanup error: ${cleanupError.message}`, 'warning'); } } } }
    ...
    <body>
        <div class="container">
            ...
            <div class="grid grid-3">
                ...
                <div class="card"><h2>Quick Actions</h2><div class="btn-group" style="flex-direction: column;"><button onclick="runInventorySync()" class="btn btn-primary" ${isSystemLocked ? 'disabled' : ''}>🔄 Run Inventory Sync</button><button onclick="runFullImport()" class="btn btn-primary" ${isSystemLocked ? 'disabled' : ''}>📦 Run Full Import</button><button onclick="clearLogs()" class="btn btn-secondary" ${isSystemLocked ? 'disabled' : ''}>🗑️ Clear Logs</button></div></div>
            </div>
